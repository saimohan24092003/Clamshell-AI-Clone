...
function createDomainSpecificStrategy(domain, smeResponses, contentAnalysis, index) {
    ...
    // Choose strategy based on domain - no fallback to Business & Management
    const baseStrategy = domainStrategies[domain] || {
        name: 'Custom Learning Strategy',
        type: 'adaptive_learning',
        description: `Tailored learning approach for ${domain} domain with personalized content delivery and interactive assessments.`,
        implementation: '4-6 weeks',
        benefits: ['Personalized learning path', 'Domain-specific content', 'Interactive assessments', 'Real-world applications'],
        ideal_for: ['Professionals in this domain', 'Learners seeking specialized knowledge'],
        expert_rationale: `This custom strategy is designed specifically for the ${domain} domain based on content analysis and learner needs.`,
        suitability: 85
    };
...
function createDetailedPersonalizedPrompt(contentAnalysis, smeResponses, sessionId) {
    ...
    // Extract Pre-SME context from global session data for dynamic personalization
    const sessionData = global.sessionData?.[sessionId];
    const preSMEAnswers = sessionData?.preSMEAnswers || {};
    const customerName = preSMEAnswers.customerName || 'Not specified';
    const projectName = preSMEAnswers.projectName || 'Not specified';
...
**Target Duration**: ${preSMEAnswers.courseDuration || 'Not specified'}
**Learning Objective**: ${preSMEAnswers.learningObjective || 'Not specified'}
...
- **Customize for ${customerName}** - consider their organizational context
- **Tailor for "${projectName}" project** - align with project goals and scope
- Provide measurable success metrics for ${domain}
- Include realistic implementation timelines matching ${preSMEAnswers.courseDuration || 'specified duration'}
- Demonstrate deep ${domain} expertise in every recommendation
- Reference the specific learning objective: "${preSMEAnswers.learningObjective || 'Not specified'}"

Generate 4-6 completely unique strategies following your structured format that could ONLY apply to this exact combination of customer (${customerName}), project (${projectName}), domain (${domain}), and SME inputs.
    `;
}
...
- Customer/Organization: ${preSMEContext.customerName || 'Not specified'}
- Project Name: ${preSMEContext.projectName || 'Not specified'}
