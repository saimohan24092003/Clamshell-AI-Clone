...
// Enhanced helper function to convert idealFor to array of strings for MongoDB
function convertIdealForToStrings(idealFor) {
  console.log('🔄 Converting idealFor for MongoDB storage:', typeof idealFor);

  // Handle string that might be JSON
  if (typeof idealFor === 'string') {
    try {
      // Try to parse as JSON
      const parsed = JSON.parse(idealFor);
      console.log('✅ Parsed JSON idealFor:', parsed);
      return convertIdealForToStrings(parsed); // Recursive call with parsed object
    } catch (e) {
      // If not JSON, return as single string
      console.log('⚠️ idealFor is string, not JSON:', idealFor);
      return idealFor.trim() ? [idealFor] : ['General purpose'];
    }
  }

  // Handle array (including parsed arrays)
  if (Array.isArray(idealFor)) {
    const result = idealFor.map(item => {
      if (typeof item === 'object' && item !== null) {
        return convertObjectToString(item);
      }
      return String(item).trim() || 'General purpose';
    }).filter(Boolean);
    console.log('✅ Array converted:', result);
    return result.length > 0 ? result : ['General purpose'];
  }

  // Handle object
  if (typeof idealFor === 'object' && idealFor !== null) {
    const result = convertObjectToString(idealFor);
    console.log('✅ Object converted to descriptive string');
    return [result];
  }

  // Fallback
  const fallback = String(idealFor).trim() || 'General purpose';
  console.log('✅ Fallback idealFor conversion completed');
  return [fallback];
}

// Helper to convert object to readable string
function convertObjectToString(obj) {
  const descriptions = [];

  if (obj.learnerTypes && Array.isArray(obj.learnerTypes)) {
    descriptions.push(`Learner Types: ${obj.learnerTypes.join(', ')}`);
  }

  if (obj.contentTypes && Array.isArray(obj.contentTypes)) {
    descriptions.push(`Content Types: ${obj.contentTypes.join(', ')}`);
  }

  if (obj.timeConstraints) {
    descriptions.push(`Time Constraints: ${obj.timeConstraints}`);
  }

  if (obj.complexity) {
    descriptions.push(`Complexity: ${obj.complexity}`);
  }

  return descriptions.length > 0 ? descriptions.join(' | ') : 'General purpose';
}

// Diversity filter to ensure varied strategy types
function applyDiversityFilter(sortedScores, maxRecommendations) {
  const selected = [];
  const usedCategories = new Set();

  // Define strategy categories to ensure diversity
  const categoryMap = {
    'Microlearning Strategy': 'content-delivery',
    'Story-Based Learning Strategy': 'engagement',
    'Scenario-Based Learning Strategy': 'application',
    'Case-Based Learning Strategy': 'analysis',
    'Gamification Strategy': 'motivation',
    'Blended Learning Strategy': 'format',
    'Spaced Learning Strategy': 'retention',
    'Collaborative Learning Strategy': 'social',
    'Simulation & Virtual Labs Strategy': 'practice',
    'Adaptive Learning Strategy': 'personalization',
    'Mobile Learning Strategy': 'accessibility',
    'Social Learning Strategy': 'community',
    'Assessment-Driven Strategy': 'evaluation',
    'Guided Learning Strategy': 'support',
    'Learning Through Exploration & Discovery (LEAD)': 'discovery'
  };

  console.log('🎯 Applying diversity filter to ensure varied strategy types...');

  // First pass: Select highest-scoring strategy from each category
  for (const scored of sortedScores) {
    const category = categoryMap[scored.strategy.name] || 'general';

    if (!usedCategories.has(category) && selected.length < maxRecommendations) {
      selected.push(scored);
      usedCategories.add(category);
      console.log(`✅ Selected ${scored.strategy.name} (${category}) - Score: ${scored.score}`);
    }
  }

  // Second pass: Fill remaining slots with highest-scoring unused strategies
  if (selected.length < maxRecommendations) {
    for (const scored of sortedScores) {
      if (!selected.find(s => s.strategy.name === scored.strategy.name) && selected.length < maxRecommendations) {
        selected.push(scored);
        console.log(`✅ Added ${scored.strategy.name} - Score: ${scored.score}`);
      }
    }
  }

  console.log(`🎯 Final diverse selection: ${selected.length} strategies`);
  return selected;
}
...
export function recommendStrategies(contentAnalysis, smeInterview, options = {}) {
  ...
  // Sort by score and apply diversity filter to ensure different types of strategies
  const sortedScores = scores.sort((a, b) => b.score - a.score);
  const diverseRecommendations = applyDiversityFilter(sortedScores, maxRecommendations);
  ...
  const recommendations = diverseRecommendations
    .map((item, index) => ({
      ...
      idealFor: convertIdealForToStrings(item.strategy.idealFor)
    }));
...
function calculateStrategyScore(strategy, contentAnalysis, smeInterview) {
  ...
  // Extract more detailed content characteristics
  const domain = contentAnalysis.domainClassification?.primaryDomain || '';
  const complexity = contentAnalysis.complexityAssessment?.level || contentAnalysis.complexityLevel || 'medium';
  const topics = contentAnalysis.extractedTopics || [];
  const qualityScore = contentAnalysis.qualityAssessment?.overallScore || 75;

  console.log(`🎯 Scoring strategy "${strategy.name}" for domain: ${domain}, complexity: ${complexity}`);

  // Enhanced content characteristics analysis (50% of score) - increased weight
  score += analyzeContentMatch(strategy, contentAnalysis) * 0.5;

  // Analyze SME preferences and responses (30% of score)
  score += analyzeSMEMatch(strategy, smeInterview) * 0.3;

  // Domain-specific bonus scoring (15% of score)
  score += analyzeDomainSpecificMatch(strategy, domain, topics) * 0.15;

  // Quality-based feasibility (5% of score)
  score += analyzeQualityBasedFeasibility(strategy, qualityScore) * 0.05;

  console.log(`📊 Final score for "${strategy.name}": ${Math.min(score, maxScore)}`);
...
// New domain-specific matching function
function analyzeDomainSpecificMatch(strategy, domain, topics) {
  let domainScore = 0;

  // Define domain-strategy affinity mapping
  const domainAffinityMap = {
    'Healthcare & Medical Training': {
      'Simulation & Virtual Labs Strategy': 95,
      'Case-Based Learning Strategy': 90,
      'Scenario-Based Learning Strategy': 85,
      'Assessment-Driven Strategy': 80,
      'Microlearning Strategy': 70
    },
    'Business & Corporate Training': {
      'Gamification Strategy': 90,
      'Microlearning Strategy': 85,
      'Story-Based Learning Strategy': 80,
      'Collaborative Learning Strategy': 75,
      'Assessment-Driven Strategy': 70
    },
    'Technology & IT Training': {
      'Simulation & Virtual Labs Strategy': 95,
      'Guided Learning Strategy': 90,
      'Microlearning Strategy': 85,
      'Adaptive Learning Strategy': 80,
      'Mobile Learning Strategy': 75
    },
    'Education & Academic': {
      'Blended Learning Strategy': 95,
      'Spaced Learning Strategy': 90,
      'Assessment-Driven Strategy': 85,
      'Collaborative Learning Strategy': 80,
      'Adaptive Learning Strategy': 75
    },
    'Skills & Professional Development': {
      'Learning Through Exploration & Discovery (LEAD)': 90,
      'Scenario-Based Learning Strategy': 85,
      'Collaborative Learning Strategy': 80,
      'Social Learning Strategy': 75,
      'Case-Based Learning Strategy': 70
    },
    'Safety & Compliance Training': {
      'Assessment-Driven Strategy': 95,
      'Scenario-Based Learning Strategy': 90,
      'Simulation & Virtual Labs Strategy': 85,
      'Microlearning Strategy': 80,
      'Story-Based Learning Strategy': 75
    }
  };

  // Check domain affinity
  if (domainAffinityMap[domain] && domainAffinityMap[domain][strategy.name]) {
    domainScore = domainAffinityMap[domain][strategy.name];
    console.log(`🎯 Domain match for ${strategy.name} in ${domain}: ${domainScore}`);
  } else {
    // Default scoring based on content topics
    const topicsText = topics.join(' ').toLowerCase();

    if (topicsText.includes('medical') || topicsText.includes('healthcare')) {
      if (strategy.name.includes('Simulation') || strategy.name.includes('Case')) domainScore = 80;
    } else if (topicsText.includes('business') || topicsText.includes('corporate')) {
      if (strategy.name.includes('Gamification') || strategy.name.includes('Micro')) domainScore = 80;
    } else if (topicsText.includes('technology') || topicsText.includes('software')) {
      if (strategy.name.includes('Guided') || strategy.name.includes('Virtual')) domainScore = 80;
    }
  }

  return Math.min(domainScore, 100);
}

// New quality-based feasibility function
function analyzeQualityBasedFeasibility(strategy, qualityScore) {
  let feasibilityScore = 50; // Base score

  // Higher quality content can support more complex strategies
  if (qualityScore > 85) {
    if (strategy.name.includes('Simulation') || strategy.name.includes('Case') || strategy.name.includes('Scenario')) {
      feasibilityScore += 30;
    }
  } else if (qualityScore < 60) {
    // Lower quality content should use simpler strategies
    if (strategy.name.includes('Microlearning') || strategy.name.includes('Guided')) {
      feasibilityScore += 25;
    }
  }
