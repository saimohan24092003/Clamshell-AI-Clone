...
Use realistic page references and ensure activities align with the content type (${contentProfile.contentType}).`;
        ...
        try {
            ...
            if (moduleData.learningActivities) {
                moduleData.topics = moduleData.learningActivities.map(activity => ({
                    ...
                    learningFormat: strategy.primaryFormats && Array.isArray(strategy.primaryFormats) && strategy.primaryFormats.includes(activity.learningFormat)
            ...
            if (moduleData.topics) {
                moduleData.topics = moduleData.topics.map(topic => {
                    if (!strategy.primaryFormats || !Array.isArray(strategy.primaryFormats) || !strategy.primaryFormats.includes(topic.learningFormat)) {
    ...
    generateAssessmentStrategy(contentProfile, strategy) {
        ...
        if (contentProfile.assessmentNeeds && Array.isArray(contentProfile.assessmentNeeds) && contentProfile.assessmentNeeds.includes('knowledge-check')) {
            assessments.push({
    ...
    selectOptimalFormat(activityName, availableFormats) {
        ...
        if (activityName && typeof activityName === 'string' && (activityName.toLowerCase().includes('demo') || activityName.toLowerCase().includes('show'))) {
            return availableFormats && availableFormats.find(f => f.includes('Demonstration')) || (availableFormats && availableFormats[0]);
        }
        if (activityName && typeof activityName === 'string' && (activityName.toLowerCase().includes('practice') || activityName.toLowerCase().includes('exercise'))) {
            return availableFormats && availableFormats.find(f => f.includes('Simulation') || f.includes('Practice')) || (availableFormats && availableFormats[0]);
        }
        return (availableFormats && availableFormats[0]) || 'Static Screen';
    }

    generateDocumentObjective(contentProfile, strategy) {
        const approach = strategy.learningApproach || 'adaptive';
        const contentType = contentProfile.contentType || 'educational';
        const formats = strategy.primaryFormats && Array.isArray(strategy.primaryFormats)
            ? strategy.primaryFormats.slice(0, 2).join(' and ')
            : 'interactive content and assessments';

        return `This adaptive learning course employs a ${approach} instructional design approach tailored specifically for ${contentType} content. Learners will engage through ${formats}, building competency through structured modules that adapt to individual learning needs and organizational context.`;
    }
    ...
    determineContentType(professionalAnalysis, userStrategy) {
        ...
        if (domain && (domain.includes('Technology') || domain.includes('IT'))) return 'technical';
        if (domain && (domain.includes('Compliance') || domain.includes('Regulatory'))) return 'compliance';
        if (domain && (domain.includes('Management') || domain.includes('Business'))) return 'conceptual';
...
- Create professional, implementation-ready content`;
        ...
        try {
            ...
            if (responseContent.includes('```json')) {
                const startIndex = responseContent.indexOf('```json') + 7;
                const endIndex = responseContent.lastIndexOf('```');
                if (endIndex > startIndex) {
                    responseContent = responseContent.substring(startIndex, endIndex).trim();
                }
            ...
            const learningMap = JSON.parse(responseContent);
    ...
    selectStrategySpecificFormat(topicName, strategy, availableFormats) {
        ...
        if (topicName && typeof topicName === 'string' && (topicName.toLowerCase().includes('introduction') || topicName.toLowerCase().includes('overview'))) {
            return formats && formats.find(f => f.includes('Visualization') || f.includes('Guide')) || (formats && formats[0]);
        }
        if (topicName && typeof topicName === 'string' && (topicName.toLowerCase().includes('practice') || topicName.toLowerCase().includes('application'))) {
            return formats && formats.find(f => f.includes('Simulation') || f.includes('Scenario')) || (formats && formats[1]);
        }
