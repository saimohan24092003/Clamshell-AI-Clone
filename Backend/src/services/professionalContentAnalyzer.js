...
Format your response as a professional instructional design report with clear sections and specific, actionable recommendations.
    ...
    /**
     * Extract domain classification with reasoning - Enhanced accuracy
     */
    extractDomainClassification(response) {
        ...
        // Enhanced domain detection with keywords
        const domainKeywords = {
            'Healthcare & Medical': ['healthcare', 'medical', 'clinical', 'patient', 'hospital', 'nurse', 'doctor', 'treatment', 'diagnosis', 'pharmaceutical', 'surgery', 'therapy'],
            'Technology & IT': ['technology', 'software', 'programming', 'code', 'development', 'system', 'network', 'database', 'cybersecurity', 'cloud', 'api', 'algorithm'],
            'Finance & Banking': ['finance', 'financial', 'banking', 'investment', 'accounting', 'budget', 'revenue', 'profit', 'loan', 'credit', 'insurance', 'audit'],
            'Education & Academic': ['education', 'academic', 'curriculum', 'pedagogy', 'teaching', 'learning', 'student', 'university', 'research', 'study', 'course', 'instruction'],
            'Manufacturing & Operations': ['manufacturing', 'production', 'operations', 'factory', 'quality control', 'supply chain', 'logistics', 'equipment', 'maintenance', 'safety'],
            'Compliance & Regulatory': ['compliance', 'regulatory', 'legal', 'regulation', 'policy', 'audit', 'risk', 'governance', 'standards', 'procedures'],
            'Sales & Marketing': ['sales', 'marketing', 'advertising', 'promotion', 'customer', 'client', 'revenue', 'campaign', 'brand', 'market research'],
            'Human Resources': ['human resources', 'hr', 'recruitment', 'hiring', 'employee', 'training', 'performance', 'workplace', 'benefits', 'compensation'],
            'Business & Management': ['business', 'management', 'leadership', 'strategy', 'operations', 'planning', 'organization', 'team', 'process', 'project']
        };
        ...
        for (const [domain, keywords] of Object.entries(domainKeywords)) {
            ...
            if (normalizedScore > bestScore && score >= 2) { // Require at least 2 keyword matches
                bestScore = normalizedScore;
                bestDomain = domain;
                confidence = Math.min(95, Math.round(normalizedScore * 100 + 50)); // Scale confidence
                reasoning = `Classified as ${domain} based on keywords: ${matchedKeywords.slice(0, 5).join(', ')}`;
            }
        }

        // Fallback to explicit domain mentions if keyword analysis fails
        if (bestDomain === 'Unknown') {
            ...
            for (const domain of domains) {
                if (responseText.includes(domain.toLowerCase())) {
                    bestDomain = domain;
                    confidence = 75;
                    reasoning = `Explicit mention of ${domain} found in analysis`;
                    break;
                }
            }
        }

        // Extract confidence if explicitly mentioned in response
        const confidenceMatch = response.match(/confidence[:\s]*(\d+)%/i);
        if (confidenceMatch) {
            confidence = Math.max(confidence, parseInt(confidenceMatch[1]));
        }
        ...
        return {
            ...
            subDomain: this.extractSubDomain(response, bestDomain)
        };
    }
